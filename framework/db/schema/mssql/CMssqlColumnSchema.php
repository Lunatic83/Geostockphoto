...
class CMssqlColumnSchema extends CDbColumnSchema
{
	...
	protected function extractType($dbType)
	{
		if(strpos($dbType,'float')!==false || strpos($dbType,'real')!==false)
			$this->type='double';
		else if(strpos($dbType,'nvarchar')!==false || strpos($dbType,'bigint')!==false || (strpos($dbType,'int')!==false || strpos($dbType,'smallint')!==false || strpos($dbType,'tinyint')))
			$this->type='integer';
	...
	protected function extractDefault($defaultValue)
	{
		...
		else if($defaultValue=null) //ADDED BY MARCO
			parent::extractDefault(null);
		else
