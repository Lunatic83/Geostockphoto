...
class CAssetManager extends CApplicationComponent
{
	...
	public function setBasePath($value)
	{
		...
		elseif(($basePath=realpath($value))!==false && is_dir($basePath) && (Yii::app()->params['useBlob'] || is_writable($basePath)))
	...
	public function publish($path,$hashByName=false,$level=-1,$forceCopy=false)
	{
		...
		else if(($src=realpath($path))!==false)
		{
			if(is_file($src))
			{
				...
				$dstUrl=$this->getBaseUrl().'/'.$dir; //ADDED
				...
				if(!Yii::app()->params['useBlob'])
					...
					if(!CFileHelper::checkBlob('/assets/'.$dir."/".$fileName) || $forceCopy)
						CFileHelper::addToBlob($src, "/assets/".$dir."/".$fileName);
					if(Yii::app()->params['useStorageEmulator'])
						return $this->_published[$path]="http://127.0.0.1:10000/devstoreaccount1/assets1//assets/"."$dir/$fileName";
					else
						return $this->_published[$path]="https://assets1.blob.core.windows.net/assets1//assets/"."$dir/$fileName";
				...
				if($this->linkAssets){
					...
					if(!Yii::app()->params['useBlob']){
						if(!is_dir($dstDir) || $forceCopy)
							CFileHelper::copyDirectory($src,$dstDir,array('exclude'=>$this->excludeFiles,'level'=>$level), $dstUrl);
					}else{
						if(!CFileHelper::checkBlob('/assets/'.$dir) || $forceCopy)
							CFileHelper::copyDirectory($src,$dstDir,array('exclude'=>$this->excludeFiles,'level'=>$level), $dstUrl);
					}
				}
						
				if(!Yii::app()->params['useBlob'])
					return $this->_published[$path]=$dstUrl;
				else{
					if(Yii::app()->params['useStorageEmulator'])
						return $this->_published[$path]="http://127.0.0.1:10000/devstoreaccount1/assets1//assets/".$dir;
					else
						return $this->_published[$path]="https://assets1.blob.core.windows.net/assets1//assets/".$dir;
				}
			}
		}
