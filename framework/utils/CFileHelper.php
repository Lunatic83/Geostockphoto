...
class CFileHelper
{
	 ...
	 * </ul>
	 */
	public static function copyDirectory($src,$dst,$options=array(),$dstUrl=null)
	{
		if(Yii::app()->params['useBlob']){
			//scrivi un blob vuoto con il nome della cartella da copiare
			//putBlobData
			self::addToBlob('',$dstUrl,true);
		}
		$fileTypes=array();
		$exclude=array();
		$level=-1;
		extract($options);
		self::copyDirectoryRecursive($src,$dst,'',$fileTypes,$exclude,$level,$dstUrl);
	}
	...
	// ADDED by Marco
	public function checkBlob($dst){
		if(Yii::app()->params['useStorageEmulator']){
			$blobStorageClient = new Microsoft_WindowsAzure_Storage_Blob();
		}else{
			$blobStorageClient = new Microsoft_WindowsAzure_Storage_Blob(
				'blob.core.windows.net',
				STORAGE_ACCOUNT_NAME_ASSETS,
				STORAGE_ACCOUNT_KEY_ASSETS
			);
		}
		
		$blobStorageClient->createContainerIfNotExists(BLOB_CONTAINER_ASSETS);
		$blobStorageClient->setContainerAcl(
			BLOB_CONTAINER_ASSETS,
			Microsoft_WindowsAzure_Storage_Blob::ACL_PUBLIC
		);
		
		return $blobStorageClient->blobExists(
        	BLOB_CONTAINER_ASSETS,
        	$dst
		);
	}
	...
	// ADDED by Marco
	public function addToBlob($src, $dest, $putData=false){
		if(Yii::app()->params['useStorageEmulator']){
			$blobStorageClient = new Microsoft_WindowsAzure_Storage_Blob();
		}else{
			$blobStorageClient = new Microsoft_WindowsAzure_Storage_Blob(
	 ...
	 * level N means copying those directories that are within N levels.
	 */
	protected static function copyDirectoryRecursive($src,$dst,$base,$fileTypes,$exclude,$level,$dstUrl=null)
	{
		if(!Yii::app()->params['useBlob']){
			@mkdir($dst);
			@chmod($dst,0777);
		...
		while(($file=readdir($folder))!==false)
		{
			...
			if(self::validatePath($base,$file,$isFile,$fileTypes,$exclude))
			{
				if($isFile){
					...
					elseif($dstUrl!=null)
						self::addToBlob($path,$dstUrl.'/'.$file);
				}else if($level)
					self::copyDirectoryRecursive($path,$dst.DIRECTORY_SEPARATOR.$file,$base.'/'.$file,$fileTypes,$exclude,$level-1,$dstUrl.'/'.$file);
			}
		}
		closedir($folder);
	}
